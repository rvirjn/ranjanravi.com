...
(function nativeApp(global) {
  ...
  function livePage() {
    ...
    if (path.includes("/remedy") || file === "remedy.html") return "remedy";
    if (path.includes("/auspicious") || file === "auspicious.html") return "auspicious";
  ...
  function refreshHeaderAuth() {
    ...
    if (avatar) {
      avatar.textContent = user ? initials(user.name || user.mobile || "PR") : "IN";
  ...
  function renderDrawer() {
    ...
    const aboutItems = HOUSES.map(
      (house) => `
        <a class="app-menu-item" href="${pageHref("kundali.html")}?house=${house.n}">
          ${icon(house.icon)}
    ...
    drawer.innerHTML = `
      ...
      <div class="app-user-card">
        <span class="app-avatar">${initials(user?.name || chart?.name || "RR")}</span>
        <div>
          <strong>${user?.name || chart?.name || "Guest"}</strong>
          <span>${chart ? "Active chart • explore houses" : "No chart yet • add birth details"}</span>
        </div>
      </div>
      <p class="app-menu-label">About your</p>
      <div class="app-menu-list">${aboutItems}</div>
  ...
  function mountProfiles() {
    ...
    mask.innerHTML = `<div class="app-sheet" role="dialog" aria-labelledby="app-profiles-title">
      ...
      <div class="app-sheet__head">
        <h2 id="app-profiles-title">Your Profiles</h2>
        <button type="button" class="app-sheet__close" id="app-sheet-close" aria-label="Close">×</button>
      </div>
      <div id="app-profile-list"></div>
      <p class="app-sheet-hint">Long-press a chart to delete it.</p>
      <button type="button" class="app-add-chart" id="app-add-chart">+ Add New Chart</button>
    </div>`;
  ...
  function renderProfiles() {
    ...
    if (!views.length) {
      list.innerHTML = `<p class="app-empty">No saved charts yet. Add a birth chart to open kundali, dasha, and remedies.</p>`;
      return;
    }
    list.innerHTML = views
      .map((view) => {
        ...
        return `<button type="button" class="app-profile-row${on ? " is-on" : ""}" data-name="${key}">
          <span class="app-avatar">${initials(view.name)}</span>
          <span><strong>${view.name}</strong><span>${view.date || ""}</span></span>
          <span class="app-edit" data-edit="${key}" aria-hidden="true">✎</span>
    ...
    list.querySelectorAll(".app-profile-row").forEach((row) => {
      row.addEventListener("click", (event) => {
        ...
        if (event.target.closest("[data-edit]")) {
          closeProfiles();
          window.location.href = `${pageHref("kundali.html")}?mode=open&chart=${encodeURIComponent(key || "")}#app=kundali`;
          return;
        }
      ...
      row.addEventListener("contextmenu", (event) => {
        event.preventDefault();
        window.alert("Charts are saved with your account. Add a new chart from Kundali to replace an old name.");
      });
    });
  }
  ...
  function fillGuidance(payload) {
    const snap = payload ? dashaSnapshot(payload) : null;
    const maha = snap?.current?.mahadashaName || "";
    const antar = snap?.current?.antardashaName || "";
    const prat = snap?.current?.pratyantardashaName || "";
  ...
  async function initDasha() {
    ...
    try {
      ...
      const moonSign = moonRow?.house_rashi || moonRow?.rashi || "Cancer";
      host.innerHTML = `
        ...
        <div class="app-accordion">
          <div class="app-acc-item">
            ...
            <div class="app-acc-body">${moonSign ? `Moon in ${moonSign} • ` : ""}${g.antar || "Mercury"} active • ${g.maha || "Jupiter"} supportive • ${g.prat || "Saturn"} steady. ${g.copy.hero}</div>
          </div>
          <div class="app-acc-item">
            <button type="button" class="app-acc-btn" data-acc="0" aria-expanded="false">Next dasha change ${ICONS.down}</button>
            <div class="app-acc-body" hidden>Next change around ${g.until}. Keep routines steady until then.</div>
          </div>
        </div>
      `;
      bindAccordions(host);
  ...
  function enhanceKundaliForm() {
    ...
    wrapIcon(document.getElementById("place-preset")?.closest(".form-field"), icon("home"));
  ...
  function enhanceRemedy() {
    ...
    intro.innerHTML = `<span class="app-kicker">Support</span><h1 class="app-title">Remedy</h1><p class="app-lead">Choose a house area for planet and ritual remedies.</p>`;
    const grid = document.createElement("div");
    grid.id = "app-remedy-grid";
    grid.className = "app-house-grid auspicious-grid";
    ...
    shell.insertBefore(intro, shell.firstChild);
    if (form) form.before(grid);
    else shell.appendChild(grid);
    grid.querySelectorAll(".feature-tile").forEach((tile) => {
      tile.addEventListener("click", () => {
        ...
        const house = tile.getAttribute("data-house");
        ...
        const openTab = document.getElementById("tab-open-birth");
        if (openTab) openTab.click();
        const select = document.getElementById("saved-birth-select");
        if (select) {
  ...
  function enhanceAuspicious() {
    ...
    const first = options.querySelector(".feature-tile");
    if (first) first.classList.add("feature-tile--active");
    options.querySelectorAll(".feature-tile").forEach((tile) => {
      tile.addEventListener("click", () => {
        options.querySelectorAll(".feature-tile").forEach((other) => other.classList.remove("feature-tile--active"));
  ...
  function injectAppScreens() {
    ...
    home.innerHTML = `
      ...
      <div class="app-section-head">
        <h2>About</h2>
        <a class="app-more" data-app-screen="kundali" href="${screenHref("kundali")}" aria-label="Open kundali"><svg viewBox="0 0 24 24"><path d="M9 5l7 7-7 7"/></svg></a>
      </div>
      <div class="app-about-row">
        <a class="app-about-item" href="${pageHref("kundali.html")}?house=1#app=kundali">
          <svg viewBox="0 0 24 24"><path d="M12 20s-6.5-4.2-6.5-8.8A3.4 3.4 0 0 1 12 8.4a3.4 3.4 0 0 1 6.5 2.8C18.5 15.8 12 20 12 20z"/></svg>
          Health
        </a>
        <a class="app-about-item" href="${pageHref("kundali.html")}?house=2#app=kundali">
          <svg viewBox="0 0 24 24"><circle cx="12" cy="12" r="7"/><path d="M12 8v8"/></svg>
          Wealth
        </a>
        <a class="app-about-item" href="${pageHref("kundali.html")}?house=7#app=kundali">
          <svg viewBox="0 0 24 24"><circle cx="9" cy="8.2" r="2.2"/><circle cx="15.2" cy="8.6" r="1.8"/><path d="M4.8 18.2c.7-2.8 2.2-4.2 4.4-4.2s3.7 1.4 4.4 4.2"/></svg>
          Relationship
        </a>
        <a class="app-about-item" href="${pageHref("kundali.html")}?house=5#app=kundali">
          <svg viewBox="0 0 24 24"><circle cx="8" cy="9" r="1.6"/><circle cx="16" cy="9" r="1.6"/><circle cx="12" cy="7.2" r="1.5"/><path d="M6.2 18.5c.6-2.6 1.8-3.8 3.5-3.8.8 0 1.5.3 2.3.9.8-.6 1.5-.9 2.3-.9 1.7 0 2.9 1.2 3.5 3.8"/></svg>
          Children
        </a>
      </div>

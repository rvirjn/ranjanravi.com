...
/** ``planet → { friends, enemies, neutrals }`` from planet-database (natural friendship). */
let planetFriendshipLookupCache = null;

function getPlanetFriendshipLookup() {
  if (planetFriendshipLookupCache) return planetFriendshipLookupCache;
  const table = {};
  for (const planet of planetDatabase?.planets || []) {
    const key = normalizeText(planet?.name);
    if (!key || key === "ascendant") continue;
    const toSet = (field) => {
      const raw = planet?.[field] || planet?.[field.charAt(0).toUpperCase() + field.slice(1)] || [];
      return new Set(
    ...
    table[key] = {
      friends: toSet("friends"),
      enemies: toSet("enemies"),
      neutrals: toSet("neutral")
    };
  }
  planetFriendshipLookupCache = table;
  return table;
}

/**
 * Natural friendship of ``subjectPlanet`` toward ``otherPlanet``:
 * friend | enemy | neutral | "" (unknown / same planet).
 */
function naturalFriendshipKind(subjectPlanet, otherPlanet) {
  const subject = normalizeText(subjectPlanet);
  const other = normalizeText(otherPlanet);
  if (!subject || !other || subject === other) return "";
  const row = getPlanetFriendshipLookup()[subject];
  if (!row) return "";
  if (row.friends.has(other)) return "friend";
  if (row.enemies.has(other)) return "enemy";
  if (row.neutrals.has(other)) return "neutral";
  return "";
}
...
async function ensurePlanetDatabase() {
  ...
  try {
    ...
    planetFriendshipLookupCache = null;
...
/** Aspected By cell: each aspector tinted by natural friendship to the row planet. */
function appendPlanetsAspectedByCell(tr, rowData) {
  ...
  const rowPlanet = normalizeText(rowData?.planet);
...
function chartPlanetDegreeInSign(p) {
  ...
  if (phase && typeof phase === "object") {
    const deg = phase.at_birth_degrees_in_sign;
    if (typeof deg === "number" && Number.isFinite(deg)) return deg;
  }
  if (typeof p?.degree_in_rashi === "number" && Number.isFinite(p.degree_in_rashi)) {
    return p.degree_in_rashi;
  }
  return null;
}

function formatChartPlanetDegree(deg) {
...
function chartPlanetDignityKind(entry) {
  const dignity = normalizeText(entry?.planet_dignity);
  if (dignity === "exalted") return "exalted";
  if (dignity === "debilitated") return "debilitated";
  const status = normalizeText(entry?.planet_status_in_rashi);
  if (status === "high") return "exalted";
  if (status === "low") return "debilitated";
  return "";
}
...
function chartPlanetExponentParts(entry) {
  ...
  if (degreeText) {
    parts.push({
      text: degreeText,
      className: "kundali-chart-planet kundali-chart-planet-exponent kundali-chart-planet-degree",
      colorFromPlanet: true
    });
  }
  const dignity = chartPlanetDignityKind(entry);
  if (dignity === "exalted") {
    parts.push({
      text: "↑",
      className:
        "kundali-chart-planet-exponent kundali-chart-planet-power kundali-chart-planet-power--exalted"
    });
...
const CHART_PLANET_EXPONENT_DY = "-2.05";
const CHART_PLANET_EXPONENT_RESET_DY = "2.05";

function appendChartPlanetExponent(textEl, entry) {
  const parts = chartPlanetExponentParts(entry);
  if (!parts.length) return;
  parts.forEach((part, i) => {
    const el = document.createElementNS("http://www.w3.org/2000/svg", "tspan");
    if (part.colorFromPlanet) {
      applyPlanetChartColor(el, entry);
      el.classList.add("kundali-chart-planet-exponent", "kundali-chart-planet-degree");
    } else {
      el.setAttribute("class", part.className);
    }
    if (i === 0) el.setAttribute("dy", CHART_PLANET_EXPONENT_DY);
    el.textContent = part.text;
    textEl.appendChild(el);
  });
  const reset = document.createElementNS("http://www.w3.org/2000/svg", "tspan");
  reset.setAttribute("dy", CHART_PLANET_EXPONENT_RESET_DY);
  reset.textContent = "\u200b";
  textEl.appendChild(reset);
}

function appendSittingPlanetTspans(textEl, entry, prefix, layout, options) {
  const label = document.createElementNS("http://www.w3.org/2000/svg", "tspan");
  if (layout.x != null) label.setAttribute("x", String(layout.x));
  if (layout.dy != null) label.setAttribute("dy", layout.dy);
  label.textContent = prefix + entry.label;
  stylePlanetTspan(label, entry, options.strengthMax);
  textEl.appendChild(label);

  if (!options.showHouseMarks) return;
  appendChartPlanetExponent(textEl, entry);
}
...
function appendColoredPlanetsToText(textEl, planets, anchorX, options = {}) {
  const showHouseMarks = Boolean(options.showHouseMarks);
  const {
    rowMode = false,
    firstDy = "0",
    rowDy = showHouseMarks ? "1.2em" : "1.05em",
    gap = showHouseMarks ? "  " : " ",
    leadGap = false,
    strengthMax
  } = options;
    ...
    rows.forEach((row, ri) => {
      row.forEach((entry, pi) => {
        ...
        if (pi === 0 && anchorX != null) {
          layout.x = anchorX;
          layout.dy = ri === 0 ? firstDy : rowDy;
        } else if (pi === 0) {
          layout.dy = ri === 0 ? firstDy : rowDy;
        }
        const prefix = pi > 0 || (leadGap && pi === 0) ? gap : "";
        appendSittingPlanetTspans(textEl, entry, prefix, layout, planetOpts);
      });
    });
    return;
  }
  ...
  list.forEach((entry, i) => {
    ...
    const prefix = i > 0 || (leadGap && i === 0) ? gap : "";
    appendSittingPlanetTspans(textEl, entry, prefix, layout, planetOpts);
  });
}
...
function appendPlanetsPlanetCell(tr, rowData, cellStyles) {
  ...
  nameEl.className = "planets-planet-name planets-strength-main";
  ...
  if (strengthText && strengthText !== "—") {
    ...
    strengthEl.className = "planets-planet-strength planets-strength-main";
    strengthEl.textContent = strengthText;
    td.appendChild(strengthEl);
  }
  ...
  applyPlanetTableCellStyle(
    td,
    cellStyles?.strength || cellStyles?.planet || "",
    "strength"
  );
  tr.appendChild(td);
}
...
/** Nakshatra name + Nakshatra Status (with strength rules) in one cell. */
function appendPlanetsNakshatraCell(tr, rowData, cellStyles) {
  const td = document.createElement("td");
  td.className = "planets-td-nakshatra";

  const nameText =
    formatTableCellForDisplay("nakshatra", planetsTableCellText("nakshatra", rowData)) || "—";
  const nameEl = document.createElement("div");
  nameEl.className = "planets-nakshatra-name planets-strength-main";
  nameEl.textContent = nameText;
  td.appendChild(nameEl);

  for (const rule of getStrengthRulesForTableColumn(rowData, "nakshatra")) {
    const line = document.createElement("div");
    line.className = "planets-strength-rule-line";
    line.textContent = formatStrengthRuleDisplayLabel(rule);
    appendStrengthPercentChangeLabel(line, rule.value);
  ...
  const statusRaw = planetsTableCellText("planet_status_in_nakshatra", rowData);
  const statusText = formatTableCellForDisplay("planet_status_in_nakshatra", statusRaw);
  if (statusText && statusText !== "—") {
    appendPlanetsTableCellWithStrengthRules(
      td,
      rowData,
  ...
  applyPlanetTableCellStyle(
    td,
    cellStyles?.planet_status_in_nakshatra || cellStyles?.nakshatra || "",
    "planet_status_in_nakshatra"
  );
  tr.appendChild(td);
}

/** House Rashi name + Rashi Status (with strength rules) in one cell. */
function appendPlanetsHouseRashiCell(tr, rowData, cellStyles) {
  const td = document.createElement("td");
  td.className = "planets-td-house-rashi";

  const rashiText =
    formatTableCellForDisplay("house_rashi", planetsTableCellText("house_rashi", rowData)) ||
    "—";
  const nameEl = document.createElement("div");
  nameEl.className = "planets-house-rashi-name planets-strength-main";
  nameEl.textContent = rashiText;
  td.appendChild(nameEl);

  const statusRaw = planetsTableCellText("planet_status_in_rashi", rowData);
  const statusText = formatTableCellForDisplay("planet_status_in_rashi", statusRaw);
  if (statusText && statusText !== "—") {
    appendPlanetsTableCellWithStrengthRules(
      td,
      rowData,
  ...
  applyPlanetTableCellStyle(
    td,
    cellStyles?.planet_status_in_rashi || cellStyles?.house_rashi || "",
    "planet_status_in_rashi"
  );
  tr.appendChild(td);
}

/** House Lord name + House Lord Strength in one cell. */
function appendPlanetsHouseLordCell(tr, rowData, cellStyles, allRows) {
  const td = document.createElement("td");
  td.className = "planets-td-house-lord";

  const lordText =
    formatTableCellForDisplay("house_lord", planetsTableCellText("house_lord", rowData)) ||
    "—";
  const nameEl = document.createElement("div");
  nameEl.className = "planets-house-lord-name planets-strength-main";
  nameEl.textContent = lordText;
  td.appendChild(nameEl);

  const strengthText = houseLordStrengthCellText(rowData, allRows);
  if (strengthText && strengthText !== "—") {
    const strengthEl = document.createElement("div");
    strengthEl.className = "planets-house-lord-strength planets-strength-main";
    strengthEl.textContent = strengthText;
    td.appendChild(strengthEl);
  }
  ...
  applyPlanetTableCellStyle(
    td,
    houseLordStrengthColorKind(rowData, allRows) || cellStyles?.house_lord || "",
    "strength"
  );
  tr.appendChild(td);
}
...
function planetsTableRowDisplayForColumn(rowData, col) {
  if (col.key === "planet") {
    ...
    if (!text || text === "No planet") return "";
    const strength = planetsTableStrengthCellText(rowData);
    if (strength && strength !== "—") return `${text} (${strength})`;
    return text;
  }
  if (col.type === "house") {
    const { number: num, for: forRaw } = houseFromTableRow(rowData);
  ...
  if (col.key === "nakshatra") {
    const name =
      formatTableCellForDisplay("nakshatra", planetsTableCellText("nakshatra", rowData)) || "";
    const status = formatTableCellForDisplay(
      "planet_status_in_nakshatra",
      planetsTableCellText("planet_status_in_nakshatra", rowData)
    );
    if (name && status && status !== "—") return `${name} (${status})`;
    return name || status || "—";
  }
  if (col.key === "house_rashi") {
    ...
    const status = formatTableCellForDisplay(
      "planet_status_in_rashi",
      planetsTableCellText("planet_status_in_rashi", rowData)
    );
    if (name && status && status !== "—") return `${name} (${status})`;
    return name || status || "—";
  }
  if (col.key === "house_lord") {
    const name =
      formatTableCellForDisplay("house_lord", planetsTableCellText("house_lord", rowData)) ||
      "";
    const strength = houseLordStrengthCellText(rowData);
    if (name && strength && strength !== "—") return `${name} (${strength})`;
...
function houseColumnDisplayForRows(houseRows, col) {
  ...
  if (col.key === "planet") {
    const names = planetRows
      .map((row) => formatTableCellForDisplay("planet", row.planet))
      .filter((name) => name && name !== "No planet");
...
function houseAspectedByFromHouseContext(houseRows, descriptions, houseNum) {
  ...
  if (sample) {
    ...
    if (planets.length) {
      return {
        planets,
        text: String(sample.aspected_by || "").trim() || formatAspectedByPlanets(sample)
      };
    }
  }
  return { planets: [], text: "" };
...
function withHouseAspectedBy(rowData, houseAspect) {
  ...
  return {
    ...rowData,
    aspected_by_planets: [...houseAspect.planets],
    aspected_by: houseAspect.text || formatAspectedByPlanets({
      aspected_by_planets: houseAspect.planets
    })
  };
}
...
function renderStatusGridDescriptionSections(descEl, entry, options = {}) {
  ...
  const houseAspect = options.houseAspectedBy || null;
  if (sections.length) {
    for (const section of sections) {
...
function renderHouseGridDescription(descEl, descriptions, houseNum, options = {}) {
  ...
  const houseAspectedBy =
    options.houseAspectedBy ||
    houseAspectedByFromHouseContext(options.houseRows, descriptions, houseNum);
  if (
    renderStatusGridDescriptionSections(descEl, entry, {
      ...options,
      houseAspectedBy
    })
  ) {
...
function createHousePlanetsSheetElement(houseNum, strengthText, houseRows, descriptions, options = {}) {
  ...
  const allRows = Array.isArray(options.allRows) ? options.allRows : [];
  const rows = Array.isArray(houseRows) ? houseRows : [];
  const houseAspectedBy = houseAspectedByFromHouseContext(rows, descriptions, houseNum);
...
function renderPlanetsTableWithColors(tbody, rows, options = {}) {
  ...
  for (const rowData of sortedRows) {
    ...
    for (const col of columns) {
      ...
      if (col.key === "house_lord") {
        appendPlanetsHouseLordCell(tr, rowData, cellStyles, lookupRows);
        continue;
      }
      if (col.key === "nakshatra") {
        appendPlanetsNakshatraCell(tr, rowData, cellStyles);
        continue;
      }
      if (col.key === "house_rashi") {
...
function renderKundaliChart(chartData, chartHost = "kundali-chart", options = {}) {
  const host =
  ...
  const showHouseMarks = options.showHouseMarks !== false;
  ...
  for (const cell of chartData.cells) {
    ...
    const densePlanets = showHouseMarks ? housePlanets.length > 2 : rowCount > 2;
    ...
    if (cell.house === 1 || cell.house === 2 || cell.house === 12) {
      ...
      if (showHouseMarks) {
        ...
        if (rashiNum != null) {
          ...
          signTspan.setAttribute("x", String(labelX));
        ...
        if (housePlanets.length) {
          appendColoredPlanetsToText(lineEl, housePlanets, rashiNum != null ? null : labelX, {
            firstDy: "0",
            gap: " ",
...
const DASHA_MONTH_LABELS = ["Jan", "Feb", "Mar", "Apr", "May", "Jun", "Jul", "Aug", "Sep", "Oct", "Nov", "Dec"];
...
function clampDashaAge(ageYears) {
  return Math.min(DASHA_AGE_LINE_MAX, Math.max(0, Number(ageYears) || 0));
}

function dashaAgeLifePercent(ageYears) {
  return `${Math.min(100, Math.max(0, (clampDashaAge(ageYears) / DASHA_AGE_LINE_MAX) * 100))}%`;
}

function dashaYearMonthFromAge(ageYears) {
  const clamped = clampDashaAge(ageYears);
  let year = Math.floor(clamped + 1e-9);
  let month = Math.round((clamped - year) * 12);
  if (month >= 12) {
    year += 1;
    month = 0;
  }
  if (year > DASHA_AGE_LINE_MAX) {
    year = DASHA_AGE_LINE_MAX;
    month = 0;
  }
  return { year, month };
}

function dashaAgeFromYearMonth(year, month) {
...
function createDashaAgeLine(mahadashas, payload, liveAge, onPickAge) {
  let selectedAge = clampDashaAge(liveAge);
  const liveParts = dashaYearMonthFromAge(liveAge);
  const birthDate = parseBirthDateFromKundaliPayload(payload);
  ...
  for (let year = 0; year <= DASHA_AGE_LINE_MAX; year += 1) {
    ...
    const btn = document.createElement("button");
    btn.type = "button";
    btn.className = kind ? `dasha-year dasha-year--${kind}` : "dasha-year";
  ...
  nowBtn.type = "button";
  nowBtn.className = "dasha-age__tool";
  nowBtn.textContent = "Now";
  nowBtn.title = "Jump to your age now";
  ...
  function pick(ageYears) {
    selectedAge = clampDashaAge(ageYears);
    syncUi();
    onPickAge(selectedAge);
  }
  ...
  track.addEventListener("click", (event) => {
    ...
    pick(clampDashaAge(x * DASHA_AGE_LINE_MAX));
  });
  track.addEventListener("keydown", (event) => {
    const { year, month } = dashaYearMonthFromAge(selectedAge);
    let nextYearValue = year;
    if (event.key === "ArrowRight" || event.key === "ArrowUp") nextYearValue = year + 1;
    else if (event.key === "ArrowLeft" || event.key === "ArrowDown") nextYearValue = year - 1;
    else if (event.key === "Home") nextYearValue = 0;
    else if (event.key === "End") nextYearValue = DASHA_AGE_LINE_MAX;
    else return;
    event.preventDefault();
